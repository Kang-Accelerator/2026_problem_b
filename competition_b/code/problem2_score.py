...
def _intersect_polygon_with_disk(
    outer_polygon: np.ndarray, center: Sequence[float], radius: float, ngon_edges: int
) -> np.ndarray:
    """将外包与平移外接圆盘相交，统一复用问题2模型适配层。"""

    return _intersect_convex_polygon(
        np.asarray(outer_polygon, dtype=float),
        disk_polygon(center, radius, ngon_edges),
        EPS,
    )
...
def posterior_for_scenario(
    ...
    if branch == "near":
        posterior = _intersect_polygon_with_disk(outer_polygon, candidate, config.near_threshold_m, config.ngon_edges)
        return {
            "branch": branch,
            "bearing_deg": None,
...
def certify_receive_cell(
    ...
    r2_upper = np.max(np.linalg.norm(sources[:, None, :] - corners[None, :, :], axis=2), axis=1)
    ...
    while stack:
        triangle, depth = stack.pop()
        upper = max(float(np.linalg.norm(sensor - source)) for sensor in corners for source in triangle)
        d1_lower = point_to_triangle_distance(config.s1, triangle)
