...
def point_in_candidate_region(region: dict[str, Any], point: Iterable[float], tol: float = 1e-9) -> bool:
    ...
    x, y = map(float, point)
    return any(
        float(cell["bounds"][0]) - tol <= x <= float(cell["bounds"][1]) + tol
        and float(cell["bounds"][2]) - tol <= y <= float(cell["bounds"][3]) + tol
...
def _adaptive_candidate_region(
    ...
    def inspect(bounds: list[float], depth: int, base_i: int, base_j: int) -> bool:
        ...
        checks = [evaluate(point) for point in _cell_stencil(bounds)]
        passes = [bool(check["score_passed"]) for check in checks]
        all_score_pass = all(passes)
        any_score_pass = any(passes)
        if all_score_pass:
            cert = certify_receive_cell(config, outer_polygon, bounds, source_points)
            ...
            if cert["status"] == "passed":
                ...
                accepted.append({
                    ...
                    "center": {"x": float((bounds[0] + bounds[1]) / 2.0), "y": float((bounds[2] + bounds[3]) / 2.0)},
    ...
    while pending and len(examined) < config.region_max_base_cells:
        i, j = pending.pop(0)
        if (i, j) in examined:
            continue
        examined.add((i, j))
...
def solve_case(case: dict[str, Any], make_plots: bool = False, output_dir: Path | None = None) -> dict[str, Any]:
    ...
    if heuristic["thales_s2"] is not None:
        heuristic_score = score_candidate(config, outer_polygon, [heuristic["thales_s2"]["x"], heuristic["thales_s2"]["y"]], source_for_search)
        heuristic["score"] = _score_summary(heuristic_score)
    else:
...
def _empty_result(config: Problem2Config, source_summary: dict[str, Any], elapsed: float) -> dict[str, Any]:
    return {
        ...
        "heuristic_reference": {"note": "启发式参考不可用：首次精确源集合为空。", "thales_s2": None, "score": None},
        "observation_model": "conservative_lower_bound_rho=max(1000,d1); no probability prior",
...
def _plot_case(case: dict[str, Any], result: dict[str, Any], output_dir: Path) -> None:
    ...
    fig = plt.figure(figsize=(15.0, 7.0), dpi=220)
    ...
    for panel in (ax, ax_zoom):
        panel.plot(circle_x, circle_y, "k--", lw=0.9, label="目标圆域")
        panel.scatter([config.s1[0]], [config.s1[1]], marker="s", s=55, color="#D62728", label=f"首次检测点 S1=({config.s1[0]:.1f},{config.s1[1]:.1f})米", zorder=6)
        panel.add_patch(Circle(config.s1, config.rho_max, fill=False, ls=":", color="#999999", lw=0.8, label="1500米接收上界"))
        if len(outer):
            panel.add_patch(Polygon(outer, closed=True, facecolor="#4C78A8", alpha=0.25, edgecolor="#123B63", lw=1.4, label="保守外包 U_bar"))
        if len(sources):
            panel.scatter(sources[:, 0], sources[:, 1], s=9, color="#2F6B9A", alpha=0.55, label="公式生成源样本 U")
    if stencil_rows:
        ...
        heat = ax.scatter(candidate_x, candidate_y, c=score_values, cmap="YlGn_r", vmin=score_min, vmax=score_max, s=24, alpha=0.82, edgecolors="none", label="实际区域评分点（J越小越优）", zorder=3)
        ax_zoom.scatter(candidate_x, candidate_y, c=score_values, cmap="YlGn_r", vmin=score_min, vmax=score_max, s=30, alpha=0.82, edgecolors="none", zorder=3)
        ...
        if threshold is not None and score_min <= float(threshold) <= score_max:
            colorbar.ax.axhline(float(threshold), color="#C62828", lw=1.5, ls="--")
    if non_near_rows:
        xs = [float(row["x"]) for row in non_near_rows]
        ys = [float(row["y"]) for row in non_near_rows]
        for panel in (ax, ax_zoom):
            panel.scatter(xs, ys, marker="o", s=34, facecolors="#E0E0E0", edgecolors="#555555", linewidths=1.0, label="接收认证通过但非近优的搜索点" if panel is ax else None, zorder=4)
    if near_seed_rows:
        xs = [float(row["x"]) for row in near_seed_rows]
        ys = [float(row["y"]) for row in near_seed_rows]
        for panel in (ax, ax_zoom):
            panel.scatter(xs, ys, marker="*", s=105, color="#F28E2B", edgecolors="#7A3E00", linewidths=0.7, label="接收认证通过的近优种子" if panel is ax else None, zorder=7)
    if receive_failed_rows:
        ...
        for panel in (ax, ax_zoom):
            panel.scatter(failed_x, failed_y, marker="x", s=28, color="#B71C1C", linewidths=1.2, label="实际接收不合格检查点" if panel is ax else None, zorder=4)
    if outside_rows:
        outside_x = [float(row["x"]) for row in outside_rows]
        outside_y = [float(row["y"]) for row in outside_rows]
        for panel in (ax, ax_zoom):
            panel.scatter(outside_x, outside_y, marker="x", s=22, color="#777777", linewidths=0.9, label="移动预算外检查点" if panel is ax else None, zorder=3)
    candidate_cells = region.get("cells", [])
    for cell in candidate_cells:
        bounds = cell["bounds"]
        for panel in (ax, ax_zoom):
            panel.add_patch(Rectangle((bounds[0], bounds[2]), bounds[1] - bounds[0], bounds[3] - bounds[2], facecolor=(0.78, 0.10, 0.10, 0.10), edgecolor="none", zorder=4))
    boundary_segments = np.asarray(region.get("boundary_segments", []), dtype=float)
    if len(boundary_segments):
        for panel in (ax, ax_zoom):
            panel.add_collection(LineCollection(boundary_segments, colors="#C62828", linewidths=3.2, linestyles="--", zorder=6))
        ax.plot([], [], color="#C62828", lw=3.2, ls="--", label="优先推荐区域（离散近似）")
    ...
    ax_zoom.text(0.02, 0.98, f"整单元接收认证={receive_text}\n单元评分验证={score_text}\n评分检查=中心+角点+边中点\n连续评分证明=否", transform=ax_zoom.transAxes, va="top", bbox={"boxstyle": "round,pad=0.35", "fc": "#F5F9FF", "ec": "#4C78A8", "alpha": 0.94})
    for panel in (ax, ax_zoom):
    ...
    ax.set_title(f"问题2：{case_name}")
    ax_zoom.set_title("候选区域局部图")
    fig.suptitle("问题2第二检测点优先区域（离散近似）", fontsize=14, fontweight="bold")
    handles, labels = ax.get_legend_handles_labels()
    unique = dict(zip(labels, handles))
    fig.legend(unique.values(), unique.keys(), loc="lower center", bbox_to_anchor=(0.5, 0.015), ncol=4, fontsize=8, framealpha=0.94)
...
def run(input_path: Path, out_dir: Path, make_plots: bool = True) -> list[dict[str, Any]]:
