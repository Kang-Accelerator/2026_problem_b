...
def build_conservative_outer_polygon(config: Problem2Config, radius: float | None = None) -> np.ndarray:
    ...
    outer = build_regular_ngon(config.target.radius, config.ngon_edges) + config.target.center
    ...
    if len(outer) == 0:
        return outer
    distance_disk = build_regular_ngon(radius, config.ngon_edges) + config.s1
    outer = _intersect_convex_polygon(outer, distance_disk, eps)
    return clean_polygon(outer, eps_geo=eps, eps_col=eps)


def disk_polygon(center: Sequence[float], radius: float, n: int) -> np.ndarray:
    """返回平移后的外接正多边形圆盘近似。"""

    return build_regular_ngon(float(radius), int(n)) + np.asarray(center, dtype=float)
...
def posterior_outer_polygon(
    ...
    if len(result) == 0:
        return result
    return _intersect_convex_polygon(result, disk_polygon(sensor, distance_radius, ngon_edges), eps)
...
def thales_reference_point(config: Problem2Config, source_points: np.ndarray) -> dict[str, Any]:
    ...
    if len(source_points) == 0:
        return {"note": "启发式参考，不是解析最优；精确源集合为空。", "thales_s2": None, "reference_source": None}
    ...
    theta = math.radians(config.theta1_deg)
    point = config.s1 + b * np.array([math.cos(theta + math.radians(psi)), math.sin(theta + math.radians(psi))])
    return {
        "note": "启发式参考，不是解析最优；泰勒斯圆不限制搜索域。",
        "thales_s2": {"x": float(point[0]), "y": float(point[1])},
