...
from competition_b.code.problem2_solve import _cell_stencil, _json_safe, _plot_case, _plot_four_case_summary, _plot_point_groups, _region_topology, point_in_candidate_region, solve_case
...
class Problem2ValidationTests(unittest.TestCase):
    ...
    def test_four_case_summary_writes_one_shared_figure(self):
        with tempfile.TemporaryDirectory() as folder:
            output = Path(folder)
            case_id = self.normal_result["case_id"]
            _plot_four_case_summary(
                [self.normal_case] * 4,
                [self.normal_result] * 4,
                output,
                [case_id] * 4,
            )
            stem = output / "problem2_four_case_summary"
            self.assertTrue(stem.with_suffix(".jpg").exists())
            self.assertTrue(stem.with_suffix(".pdf").exists())
    ...
    def test_oblique_symmetric_case_keeps_region_points_cells_and_boundary_mirrored(self):
        case = copy.deepcopy(next(item for item in self.cases if item["case_id"] == "p2_near_branch"))
        angle_deg = 31.0
        angle = math.radians(angle_deg)
        s1 = case["S1"]
        case["S1"] = {
            "x": float(math.cos(angle) * s1["x"] - math.sin(angle) * s1["y"]),
            "y": float(math.sin(angle) * s1["x"] + math.cos(angle) * s1["y"]),
        }
        case["theta1_deg"] = float(case["theta1_deg"] + angle_deg)
        config = parse_config(case)
        self.assertTrue(has_observation_axis_symmetry(config))
        result = solve_case(case)
        region = result["candidate_region"]
        self.assertEqual(region["coordinate_frame"], "observation_axis")
        self.assertGreater(len(region["cells"]), 0)

        def point_key(point):
            return round(float(point[0]), 4), round(float(point[1]), 4)

        evaluated_by_key = {point_key((row["x"], row["y"])): row for row in region["evaluated_points"]}
        for row in region["evaluated_points"]:
            mirror = reflect_about_observation_axis(config, (row["x"], row["y"]))
            mirror_key = point_key(mirror)
            self.assertIn(mirror_key, evaluated_by_key)
            partner = evaluated_by_key[mirror_key]
            self.assertEqual(row["status"], partner["status"])
            if row.get("j_lambda") is not None and partner.get("j_lambda") is not None:
                self.assertAlmostEqual(float(row["j_lambda"]), float(partner["j_lambda"]), places=10)

        cell_keys = {
            tuple(sorted(point_key(point) for point in cell["vertices"]))
            for cell in region["cells"]
        }
        for cell in region["cells"]:
            mirrored = [reflect_about_observation_axis(config, point) for point in cell["vertices"]]
            self.assertIn(tuple(sorted(point_key(point) for point in mirrored)), cell_keys)

        segment_keys = {
            tuple(sorted((point_key(segment[0]), point_key(segment[1]))))
            for segment in region["boundary_segments"]
        }
        for segment in region["boundary_segments"]:
            mirrored = [reflect_about_observation_axis(config, point) for point in segment]
            self.assertIn(tuple(sorted((point_key(mirrored[0]), point_key(mirrored[1])))), segment_keys)

        thales_pair = result["heuristic_reference"]["thales_s2_pair"]
        self.assertEqual(len(thales_pair), 2)
        mirror = reflect_about_observation_axis(config, (thales_pair[0]["x"], thales_pair[0]["y"]))
        self.assertTrue(np.allclose(mirror, (thales_pair[1]["x"], thales_pair[1]["y"]), atol=1e-6))
