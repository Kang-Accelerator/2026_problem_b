...
class Problem2ModelTests(unittest.TestCase):
    ...
    def test_thales_reference_uses_actual_representative_source_direction(self):
        case = next(item for item in self.cases if item["case_id"] == "p2_external_tangent")
        config = parse_config(case)
        reference = thales_reference_point(config, sample_exact_source_points(config))
        s2 = np.array([reference["thales_s2"]["x"], reference["thales_s2"]["y"]], dtype=float)
        source = np.array([reference["reference_source"]["x"], reference["reference_source"]["y"]], dtype=float)
        self.assertAlmostEqual(float(np.dot(config.s1 - s2, source - s2)), 0.0, places=6)
        self.assertEqual(len(reference["thales_s2_pair"]), 2)
        for point in reference["thales_s2_pair"]:
            paired_s2 = np.array([point["x"], point["y"]], dtype=float)
            self.assertAlmostEqual(float(np.dot(config.s1 - paired_s2, source - paired_s2)), 0.0, places=6)
